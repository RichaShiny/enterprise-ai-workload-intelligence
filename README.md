...
#### Optional model-backed policy summaries
...
sends only the submitted question plus the selected approved-policy excerpts;
the service does not store question history and requests a non-stored provider
response. `GET /policy-assistant/provider-status` shows readiness without
exposing credentials. Review your organisation's data-processing requirements
...
The policy assistant retrieves a small lexical candidate set first, then can rerank those candidates with the repository’s `all-MiniLM-L6-v2` semantic reranker. Set `POLICY_SEMANTIC_RERANKING_ENABLED=true` in the service environment to enable it. Deployments that enable this opt-in mode must install `requirements-semantic-reranking.txt`; the default API image stays lean and the embedding model downloads when semantic mode starts. Reranking changes evidence order only: it cannot introduce a new policy source, bypass the evidence threshold, or override abstention. The active mode is returned as policy-response metadata and through `GET /policy-assistant/catalog`.
