...
The policy assistant retrieves a small lexical candidate set first, then can rerank those candidates with the repository’s `all-MiniLM-L6-v2` semantic reranker. Set `POLICY_SEMANTIC_RERANKING_ENABLED=true` in the service environment to enable it. The API container includes the required embedding dependency; the embedding model downloads when this opt-in mode starts. Reranking changes evidence order only: it cannot introduce a new policy source, bypass the evidence threshold, or override abstention. The active mode is returned as policy-response metadata and through `GET /policy-assistant/catalog`.
